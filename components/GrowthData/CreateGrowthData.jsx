...
const GrowthDataSchema = Yup.object().shape({
  ...
  headCircumference: Yup.number().positive(
    "Head circumference must be greater than zero"
  ),
  armCircumference: Yup.number().positive(
    "Arm circumference must be greater than zero"
  ),
});

const CreateGrowthData = ({ visible, onClose, childId, fetchGrowthData }) => {
    ...
    try {
      ...
      await api.post(`/children/${childId}/growth-data`, {
        ...
        armCircumference: values.armCircumference
          ? parseFloat(values.armCircumference)
          : undefined,
      });
      showSnackbar("Growth data added successfully!", 3000, "Close");
      fetchGrowthData();
      onClose();
    } catch (error) {
      if (
        ...
        if (Array.isArray(validationErrors)) {
          validationErrors.forEach((err) => {
            showSnackbar(`${err.error}`, 5000, "Close");
          });
        } else {
          showSnackbar(
            error.response.data.message ||
              "Failed to add growth data. Please try again.",
            5000,
            "Close"
          );
        }
      ...
      <SafeAreaProvider>
        <View style={styles(theme).modalContainer}>
          <View style={styles(theme).modalContent}>
            <Text style={styles(theme).modalTitle}>Add Growth Data</Text>
            <Formik
              initialValues={{
                inputDate: new Date(),
                height: "",
            ...
            >
              {({
                ...
                <View>
                  ...
                  {/* Input Date with DatePickerInput */}
                  ...
                  {/* Submit Button */}
                  <TouchableOpacity
                    style={[styles(theme).submitButton, { marginTop: 20 }]}
...
export default CreateGrowthData;
