...
const DoctorConsultationRequest = () => {
    ...
    const fetchConsultations = async () => {
        try {
            ...
            if (
                ...
                Array.isArray(response.data.requests.requests)
            ) {
                const allConsultations = response.data.requests.requests;
                allConsultations.sort((a, b) => new Date(b.updatedAt) - new Date(a.updatedAt));

                setConsultations(allConsultations);
                setTotalRequests(allConsultations.length || 0);
            } else {
    ...
    const handleUpdateStatus = async (consultationId, newStatus) => {
        try {
            ...
            showSnackbar(`Request ${newStatus} successfully updated!`, 5000, "Close");
    ...
    const renderConsultationItem = (item) => {
        ...
        const child = item.children?.length > 0 ? item.children[0] : {};
        const childName = child.name || "Unknown";
        const childAge = child.birthDate ? calculateAge(child.birthDate) : "Unknown Age";

        const parent = item.member || {};
            ...
            <Card style={styles(theme).consultationCard} key={item._id}>
                <Card.Content>
                    <View style={styles(theme).cardHeader}>
                        <Title style={styles(theme).doctorName}>Parent: {parentName}</Title>
                        <Text style={[styles(theme).statusText, { color: statusColor }]}>{statusText}</Text>
                    </View>

                    <View style={styles(theme).consultationDetails}>
                        <Text style={styles(theme).detailLabel}>Title:</Text>
                        <Text style={styles(theme).detailValue}>{title}</Text>

                        <Text style={styles(theme).detailLabel}>Child:</Text>
                        <Text style={styles(theme).detailValue}>{childName}</Text>

                        {child.birthDate && (
                            <>
                                <Text style={styles(theme).detailLabel}>Age:</Text>
                                <Text style={styles(theme).detailValue}>{childAge}</Text>
                            </>
                        )}

                        <Text style={styles(theme).detailLabel}>Doctor:</Text>
                        <Text style={styles(theme).detailValue}>{doctorName}</Text>

                        <Text style={styles(theme).detailLabel}>Submitted:</Text>
                        <Text style={styles(theme).detailValue}>{formatDate(item.createdAt)}</Text>

                        <Text style={styles(theme).detailLabel}>Updated:</Text>
                        <Text style={styles(theme).detailValue}>{formatDate(item.updatedAt)}</Text>
                    </View>
        ...
        <ScrollView contentContainerStyle={styles(theme).container}
            ...
            <Portal>
                <Modal
                    ...
                    <ScrollView contentContainerStyle={styles(theme).modalScrollContainer}
                        showsVerticalScrollIndicator={true}>
                            ...
                            selectedConsultation.children?.length > 0 && (
                                <View style={styles(theme).modalContent}>
                                    ...
                                    <Text style={styles(theme).modalSubtitle}>
                                        Child:{" "}
                                        {selectedConsultation.children[0].name}
                                    </Text>
                                    ...
                                    <View style={styles(theme).consultationDetails}>
                                        ...
                                        <Text style={styles(theme).detailValue}>{selectedConsultation.member?.name || "Unknown"}</Text>
                                        ...
                                        <Text style={styles(theme).detailValue}>{selectedConsultation.title || "No title provided"}</Text>


                                        <Text style={styles(theme).detailLabel}>Submitted:</Text>
                                        <Text style={styles(theme).detailValue}>{formatDate(selectedConsultation.createdAt)}</Text>
                    ...
                    {selectedConsultation.status?.toLowerCase() !== "accepted" &&
                        selectedConsultation.status?.toLowerCase() !== "rejected" && (
                            <View style={styles(theme).modalFooter}>
                                <Button
                                    mode="outlined"
                                    onPress={() => handleReject(selectedConsultation._id)}
                                ...
                                <Button
                                    mode="contained"
                                    onPress={() => handleAccept(selectedConsultation._id)}
